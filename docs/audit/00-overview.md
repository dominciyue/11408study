...
| 3 | **JWT secret 硬编码 + Actuator 全开** — `application.yml:43` 写死 base64 secret（解码即明文），SecurityConfig 把 `actuator/**` permitAll，攻击者可 `/actuator/env` 直接读 secret 后伪造任意用户 token | 🔴 阻塞 | **整个鉴权系统可被旁路** | doc 1 §3.1, doc 2 P0-07/P0-11 |
| 4 | **HTTPS / Secret / CORS 三件套未配置** — Nginx 仅 80 端口、CORS 用 `setAllowedOriginPatterns("*")` + `allowCredentials=true`、所有密码硬编码（DB/MinIO 都是 `*_dev`） | 🔴 阻塞 | 中间人攻击、跨站凭证窃取、git clone 即拿到生产凭证 | doc 1 §3.1, doc 2 P0-09/P0-07/P0-08/P0-10 |
| 5 | **AI 调用无超时 + AI 启动不校验 key + 无 API 限流** — `AiClientService` 用 `new RestTemplate()` 无 timeout、ai-service 启动不验证 LLM key（首次调用才失败 14s）、Nginx 无 limit_req | 🔴 阻塞（限流归 P1） | AI 故障级联拖垮 backend、刷 LLM 费用零拦截、暴力破解零防护 | doc 1 §3.1/§4.2, doc 2 P0-06/P0-13（限流见 P1-09）, doc 4 §1.2 |
...
| **03** | [03-production-deployment.md](./03-production-deployment.md) | 单 VPS 4C16G 生产部署完整方案（4C8G 已贴近上限）：prod compose、Nginx HTTPS、PostgreSQL 调优、备份、监控、CI/CD、容量、月成本约 ¥1600 | 5100 字 / 1054 行 |
...
| **05** | [05-local-validation.md](./05-local-validation.md) | 本地跑通验证报告：7 服务全 Up、5 个 blocker 修复（compose 端口 / pom lombok.version / Maven 镜像 / PyPI 镜像 / Node 升级）、15 个未修问题 | 2200 字 / 231 行 |
...
为完成"本地跑通"目标，本审查共 commit 5 处必要的 blocker 修复（不改业务代码）：

| commit | 改动 | 原因 |
|---|---|---|
| `67f1258` | docker-compose.yml 加 backend/frontend 端口暴露 | 便于宿主直接 curl 验证 |
| `4c600a8` | backend/pom.xml 加 `<lombok.version>1.18.32</lombok.version>` | maven-compiler-plugin 引用 `${lombok.version}` 但 properties 未定义，构建失败 |
...
第 2 项（lombok.version）和第 5 项（Node 升级）是**原代码 regression**——前一作者疏忽，已在 doc 1 §4.2/§4.3 标注。第 3、4 项是国内构建环境问题，已在 doc 2 §5.1 列入"国内构建环境标准化"建议。
...
1. **会话 A — 修真功能 Bug**：按 doc 2 P0-01/02/03/04 修 4 个真 bug（JWT userId、拓扑大小写、ImportController 鉴权、SM-2 算法）；预估 12-16 小时；产出 1 个 PR
2. **会话 B — 安全加固**：按 doc 2 P0-05~13（共 9 项：前端 Auth header / AI timeout / JWT secret / 密码外部化 / HTTPS / CORS / Actuator / Swagger 关闭 / AI 启动校验）；预估 24-40 小时；产出 1-2 个 PR
