...
- **P1-01 监控栈 done（commit 86ac0a5）**：Prometheus + Grafana + Loki + Promtail，
  micrometer-prometheus 接入 backend，prometheus-fastapi-instrumentator 接入 ai-service，
  10 面板 dashboard（HTTP/JVM/AI）；docker compose --profile monitoring up -d
- **P1-02 备份 done（commit f5498e5）**：scripts/backup.sh + restore.sh + install-backup-cron.sh；
  恢复演练已通过；详见 docs/dev-log/2026-05-18-monitoring-backup-session.md
...
2. P1-04 Prompt Redis 缓存 + P1-12 @Cacheable（成本降低 + 性能）
3. alertmanager + 5 条核心告警规则（让监控真正"会叫"）
4. postgres-exporter + redis-exporter（补全 DB/Redis 内部指标）
5. WeeklyReportServiceUnitTest pre-existing fail + 测试覆盖到 70%（CI 信号）
